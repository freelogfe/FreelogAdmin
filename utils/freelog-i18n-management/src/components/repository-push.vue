<template>
  <el-popover
    ...
    width="480"
    trigger="hover"
    :disabled="!repositoryChanges.length"
    v-model="showUpdatePopover"
  >
    <el-badge
      class="item"
      slot="reference"
    ...
    <div class="imc-repos-changes-box">
      ...
      <p
        class="imc-repos-change-item"
        v-for="(change, index) in repositoryChanges"
        :key="'change' + index"
      >
        <span :class="[change.type]">{{ change.type }}: </span>
        {{ change.path
        }}<a
          :href="`//i18n-ts.testfreelog.com/v1/file/download?repositoryName=${repositoryName}&filePath=${change.path}`"
          target="_blank"
          >下载</a
        >
      </p>
  ...
  methods: {
    async commitAndPushChanges() {
      ...
      try {
        ...
        const result = await window
          .fetch('//i18n-ts.testfreelog.com/v1/i18nRepository/changes/push', {
            method: 'POST',
            body: JSON.stringify({
    ...
    async getGithubUserInfo() {
      ...
      if (accessToken != null) {
        const result = await this.$axios
          .get(`/v1/i18n/getGithubUserInfo?accessToken=${accessToken}`)
          .then((res) => res.data);
        if (result.errcode === 0) {
    ...
    async refreshGithubAccessToken() {
      if (this.$route.query.code) {
        const result = await this.$axios
          .post('/v1/i18n/getGithubAccessToken', {
