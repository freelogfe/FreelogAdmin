  ...
  <div class="i-m-nem-tags-view">
    <div class="i-m-nem__header clearfix">
      ...
      <el-input
        ...
        @keyup.enter.native="refreshRenderedKeysListMap"
      ></el-input>
      <div class="tag-input-box">
        <el-input
          ...
          v-model="tagStr"
        >
          <el-button type="primary" slot="append" icon="el-icon-plus" @click="batchAddTag"
            >添加tag</el-button
          >
        </el-input>
      </div>
    ...
    <div v-if="selectedModuleName === ALL_MODULES">
      <el-collapse v-model="activeNames" v-if="renderedKeysListMap != null">
        <el-collapse-item
          v-show="
            renderedKeysListMap[_module.name] != null && renderedKeysListMap[_module.name].length
          "
        ...
        >
          <el-checkbox-group
            class="i-m-nem-tags-checkbox-group clearfix"
            v-model="checkedKeyListArr[i]"
            v-if="renderedKeysListMap != null"
            @change="onCheckedKeyListChange"
          >
            <el-col
              :span="12"
              v-for="(item, j) in renderedKeysListMap[_module.name]"
              :key="item.key + j"
            >
              ...
              <div class="i-m-nem-tag-group">
                <key-tags
                  ...
                  :keyInfo.sync="keyInfosMap[item.key]"
                ></key-tags>
              </div>
            </el-col>
          </el-checkbox-group>
    ...
    <div style="margin-top: 20px" v-else>
      <el-checkbox-group
        class="i-m-nem-tags-checkbox-group clearfix"
        v-model="checkedKeyListArr[0]"
        v-if="renderedKeysListMap != null"
        @change="onCheckedKeyListChange"
      >
        <el-col
          :span="12"
          v-for="(item, j) in renderedKeysListMap[selectedModuleName]"
          :key="item.key + j"
        >
          <el-checkbox :label="item.key"></el-checkbox>
          <div class="i-m-nem-tag-group">
  ...
  methods: {
    ...
    refreshRenderedKeysListMap() {
      ...
      for (const moduleName of moduleNames) {
        const moduleI18nData = this.getModuleI18nData(
          this.allModuleData,
          this.repositoryName,
          moduleName,
        );
    ...
    async fetchAllKeyInfos() {
      const result = await window
        .fetch('//i18n-ts.testfreelog.com/v1/i18nKeyInfos/all')
        .then((resp) => resp.json());
      this.keyInfosMap = {};
      if (result.errcode === 0) {
