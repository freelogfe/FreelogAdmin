  ...
  <div class="i18n-m-namespace-edit-mode">
    <div class="i-m-nem-head">
      ...
      <el-input
        ...
        @keyup.enter.native="refreshSidebar"
      >
      </el-input>
      <div class="selected-tag-box" v-if="selectedTag != ''">
        ...
        <el-tag
          ...
          closable
          @close="selectedTag = ''"
          @click="showSelectedTagInput"
          >{{ selectedTag }}</el-tag
        >
      </div>
      <el-badge
        class="save-key-changes-btn"
        slot="reference"
        :value="changedKeys.length"
        :hidden="changedKeys.length === 0"
      >
        <el-button
          type="primary"
          size="small"
      ...
      <el-button class="add-new-namescpace-btn" type="danger" size="small" @click="showNewKeyDialog"
        >新增Key</el-button
      >
      <el-button
        class="add-new-module-btn"
        type="danger"
        size="small"
        @click="newModuleDialogVisible = true"
        >新增模块</el-button
      >
    </div>
    <namespace-sidebar
    ...
    <div class="i-m-nem-body" v-if="selectedKeyItem != null">
      <div class="i-m-nem-breadcrumb-box">
        <el-breadcrumb separator="/" v-if="selectedRepoModule != null">
          <el-breadcrumb-item
            v-for="(item, index) in selectedRepoModule.keys"
    ...
    <div class="i-m-nem-empty" v-else>
      <div
        class="empty-module-box"
        v-if="selectedModuleName !== ALL_MODULES && selectedModuleIsEmpty === true"
      >
        <el-button class="add-del-module-btn" type="danger" size="small" @click="delEmptyModule"
          >删除{{ selectedModuleName }}模块</el-button
        >
        <p>当前模块为空（没有任何Key）</p>
      </div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          size="small"
          :disabled="newModuleValidtedError != ''"
          @click="addNewModule"
          >确 定</el-button
        >
      </span>
...
export default {
  ...
  watch: {
    ...
    selectedTag() {
      ...
      if (this.selectedTag !== '') {
        window
          .fetch(`//i18n-ts.testfreelog.com/v1/i18nKeyInfos/list?tags=${this.selectedTag}`)
          .then((resp) => resp.json())
          .then((res) => {
            if (res.errcode === 0) {
  ...
  methods: {
    ...
    async addNewModule() {
      ...
      if (moduleName !== '') {
        ...
        const res = await window
          .fetch('//i18n-ts.testfreelog.com/v1/i18nRepository/newModule', {
            method: 'POST',
            body: JSON.stringify({
              moduleName,
              repositoryName,
    ...
    async saveChanges() {
      ...
      const res = await window
        .fetch('//i18n-ts.testfreelog.com/v1/i18nRepository/data', {
          method: 'PUT',
          headers: {
    ...
    async handleTagInputConfirm() {
      ...
      if (newTag !== '') {
        ...
        const result = await window
          .fetch('//i18n-ts.testfreelog.com/v1/i18nKeyInfos/tags', {
            method: 'PUT',
            headers: {
              'content-type': 'application/json',
            },
